...
type IdActivationForm = z.infer<typeof formSchema>;

const IdActivationForm = () => {
  const router = useRouter();

  const [loading, setLoading] = useState(false);

  const form = useForm<IdActivationForm>({
  ...
  const onSubmit = async (data: IdActivationForm) => {
    try {
      setLoading(true);
      toast.loading("Uploading...");
      await axios.post("/api/admin/settings/idActive", data).then(() => {
        router.push("/admin/settings/id-active");
        toast.dismiss();
...
export default IdActivationForm;
