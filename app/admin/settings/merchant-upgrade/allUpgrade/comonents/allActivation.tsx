...
import { NewMerchants } from "@prisma/client";
import { useRouter } from "next/navigation";
import { Badge } from "@/components/ui/badge";
import axios from "axios";

interface MerchantUpgradeTableProps {
  initialData: NewMerchants[] | null;
}

const MerchantUpgradeTable: React.FC<MerchantUpgradeTableProps> = ({
  initialData,
}) => {
  ...
  const handleQuried = async (id: string) => {
    ...
    axios
      .post("/api/admin/manual/query/merchantUpgrade", { id })
      .then(() => {
        toast.dismiss();
        router.refresh();
  ...
  const columns: ColumnDef<NewMerchants>[] = [
    {
      id: "id",
      header: () => <div>No.</div>,
      cell: ({ row }) => (
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <Link href={`https://wa.me/+88${partner?.email}`}>
                <DropdownMenuItem>
...
export default MerchantUpgradeTable;
